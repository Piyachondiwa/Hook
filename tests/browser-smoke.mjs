...
const state = await page.evaluate(() => {
  ...
  return {
    ...
    villageRoadOpen:check(4260,1100),roadClearAfterBridge:roadXs.map(x=>({x,open:check(x,1100)})),
    houseBlocked:check(3950,920),riceBlocked:check(4050,1280),riceSamples:riceXs.map(x=>({x,blocked:!check(x,1280)})),allRiceBlocked:riceXs.every(x=>!check(x,1280)),
    fenceBlocked:check(3990,1220),fenceSamples:fenceXs.map(x=>({x,blocked:!check(x,1220)})),allTopFencesBlocked:fenceXs.every(x=>!check(x,1220)),
    outsideWestBlocked:!check(3510,1000),outsideEastBlocked:!check(5200,1000),outsideNorthBlocked:!check(4300,580),outsideSouthBlocked:!check(4300,1800),
...
if(!state.riceBlocked||!state.allRiceBlocked)throw new Error(`Player can walk through a rice field: ${JSON.stringify(state.riceSamples)}`);
if(!state.fenceBlocked||!state.allTopFencesBlocked)throw new Error(`Player can walk through a fence: ${JSON.stringify(state.fenceSamples)}`);
