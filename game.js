...
function sign(x,y){R(x,y,8,35,'#563725');R(x-4,y,58,24,'#8a5b36');R(x-1,y+4,50,16,'#a87040');R(x+6,y+6,34,2,'#d09a5e');R(x+12,y+10,4,2,'#7a492f')}
...
function hero(){const moving=p.walking,phase=p.frame,x=p.x,y=p.y;const bob=moving?(phase===1?1:0):0;const legA=moving&&phase===0?3:moving&&phase===2?-3:0;const legB=-legA;const armA=moving&&phase===0?-2:moving&&phase===2?2:0;const armB=-armA;R(x-12,y+27,24,5,'#172b20');R(x-12+legA,y+14+bob,9,13,'#292c3b');R(x+3+legB,y+14+bob,9,13,'#292c3b');R(x-14+legA,y+25+bob,12,4,'#171a25');R(x+3+legB,y+25+bob,12,4,'#171a25');R(x-11,y-3+bob,22,21,'#315e9a');R(x-7,y-8+bob,14,8,'#5e83b9');R(x-9,y+8+bob,18,9,'#243f70');R(x-9,y-20+bob,18,17,'#e6b98e');R(x-10,y-23+bob,20,7,'#3a2929');R(x-7,y-27+bob,15,7,'#51332b');R(x-5,y-25+bob,4,3,'#694333');R(x-6,y-14+bob,3,3,'#1b2230');R(x+3,y-14+bob,3,3,'#1b2230');R(x-15+armA,y-2+bob,4,17,'#e6b98e');R(x+11+armB,y-2+bob,4,17,'#e6b98e');R(x-17+armA,y+13+bob,7,5,'#315e9a');R(x+10+armB,y+13+bob,7,5,'#315e9a')}
function directionArrow(){const x=p.x,y=p.y-43;g.save();g.globalAlpha=.9;g.fillStyle='#d8e4ef';g.beginPath();if(p.dir==='up'){g.moveTo(x,y-8);g.lineTo(x-6,y);g.lineTo(x-2,y);g.lineTo(x-2,y+9);g.lineTo(x+2,y+9);g.lineTo(x+2,y);g.lineTo(x+6,y)}if(p.dir==='down'){g.moveTo(x,y+8);g.lineTo(x-6,y);g.lineTo(x-2,y);g.lineTo(x-2,y-9);g.lineTo(x+2,y-9);g.lineTo(x+2,y);g.lineTo(x+6,y)}if(p.dir==='left'){g.moveTo(x-8,y);g.lineTo(x,y-6);g.lineTo(x,y-2);g.lineTo(x+9,y-2);g.lineTo(x+9,y+2);g.lineTo(x,y+2);g.lineTo(x,y+6)}if(p.dir==='right'){g.moveTo(x+8,y);g.lineTo(x,y-6);g.lineTo(x,y-2);g.lineTo(x-9,y-2);g.lineTo(x-9,y+2);g.lineTo(x,y+2);g.lineTo(x,y+6)}g.closePath();g.fill();g.globalAlpha=.25;g.fillStyle='#fff';g.fillRect(x-1,y-1,2,2);g.restore()}
