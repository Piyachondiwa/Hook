...
function fence(x,y,w){for(let q=x;q<=x+w;q+=28){B(q,y,5,25,C.dark);B(q-2,y-3,9,4,C.wood2)}B(x,y+6,w,4,C.wood2);B(x,y+18,w,4,C.wood)}
function sakura(x,y,s=1){B(x-5*s,y,10*s,38*s,C.wood);B(x-2*s,y,5*s,31*s,C.wood2);for(const q of [[0,-35,30,18],[-31,-26,28,18],[31,-25,28,18],[0,-55,23,17],[-18,-49,21,14],[20,-47,21,14]])B(x+(q[0]-q[2]/2)*s,y+q[1]*s,q[2]*s,q[3]*s,q[0]%2?C.pink:C.pink2);for(let i=0;i<10;i++)B(x+((i*19)%64-32)*s,y+(-18-(i%5)*7)*s,4*s,4*s,C.pink)}
