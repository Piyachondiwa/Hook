/* Moonwood: organic Japanese island tree layer, pixel-only, with clear travel lanes. */
(()=>{'use strict';
...
{x:3790,y:1340,t:'pine',s:.85},{x:4090,y:1480,t:'pine',s:.72},{x:4550,y:1480,t:'maple',s:.72},{x:4880,y:1320,t:'broad',s:.78},
...
function pxCluster(cx,cy,rx,ry,seed,colors,step=4){for(let y=-ry;y<=ry;y+=step)for(let x=-rx;x<=rx;x+=step){const d=(x*x)/(rx*rx)+(y*y)/(ry*ry);if(d>1)continue;const n=Math.sin((x+seed)*12.9898+(y-seed)*78.233)*43758.5453;const f=n-Math.floor(n);const edge=d>.72;if(edge&&f<.42)continue;if(f<.10)continue;const c=f>.86?colors[2]:f>.58?colors[1]:colors[0];const z=f>.82?4:3;B(cx+x,cy+y,z,z,c);if(f>.92)B(cx+x+3,cy+y-2,2,2,colors[2])}}
...
window.moonwoodIslandTrees={count:trees.length,detail:'organic-pixel',microPixels:520,groundShadows:true,noRectangularCanopy:true,clearTravelLanes:true};
