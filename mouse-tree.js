...
function insideTree(t,x,y){const s=typeof window.treeScale==='function'?window.treeScale(t):1.5,dx=x-t.x,dy=y-(t.y-24*s),rx=54*s,ry=70*s;return dx*dx/(rx*rx)+dy*dy/(ry*ry)<=1}
...
function hover(ev){if(open)return;const q=screenToWorld(ev),t=pick(q.x,q.y);if(t){hoverCard.style.display='block';card(ev,t);canvas.style.cursor='pointer'}else{hoverCard.style.display='none';canvas.style.cursor='default'}}
