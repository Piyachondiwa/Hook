/* Moonwood: safe Japanese island expansion pass. */
(()=>{'use strict';
...
const C={grass:'#71834a',grass2:'#809553',soil:'#6b5939',soilHi:'#927844',stone:'#77746d',stoneHi:'#b8b2a4',stoneDark:'#45433f',sand:'#a9965b',sandHi:'#c0ac68',water:'#1e6478',waterHi:'#63adb8'};
...
function terrain(){
  ...
  B(3500,760,150,760,C.stoneDark);B(3520,770,145,730,C.soil);B(3550,790,120,680,C.grass);
  B(4780,760,370,760,C.stoneDark);B(4760,780,390,720,C.soil);B(4740,800,420,680,C.grass);
  B(3570,1510,1490,48,C.stoneDark);B(3550,1538,1530,46,C.stone);B(3570,1565,1490,58,C.soil);B(3590,1600,1450,120,C.grass);
  for(let i=0;i<230;i++){const x=3520+(i*73)%1570,y=650+(i*47)%1080,n=rnd(i*13,71);if(x>3780&&x<4830&&y>760&&y<1515)continue;if(n<.34)B(x,y,4+(i%4),3,C.grass2);else if(n<.55)B(x,y,5,3,C.stoneHi);else if(n<.73)B(x,y,7,3,C.soilHi);else B(x,y,3,2,C.sandHi)}
  for(let i=0;i<85;i++){const x=3510+(i*97)%1620;B(x,735+(i%3)*3,12,3,C.sandHi);B(x,1718+(i%4)*4,15,3,C.stoneHi)}
...
function expandedBlocked(x,y){
  ...
  if(inExpansion(x,y)&&!(x>=3820&&x<=4800&&y>=760&&y<=1510))return false;
  return baseBlocked?baseBlocked(x,y):true;
}
